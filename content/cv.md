---
title: "CV"
layout: "single"
url: "/cv/"
summary: "Curriculum Vitae"
ShowToc: false
ShowBreadCrumbs: false
---

## Experience

### Allen Institute for Cell Science
**Software Engineer** · 2023 — Present · Seattle, WA

Building computational methods for 3D cell imaging and microscopy. Developing knowledge graphs, foundation models, and AI-powered tools for the Cell Science division.

### Simon Fraser University
**M.Sc. Computer Science** · 2021 — 2023 · Burnaby, BC

Medical Image Analysis group under Prof. Ghassan Hamarneh. Thesis: structural analysis of endoplasmic reticulum dynamics using network-based methods. NanoscopyAI collaboration with UBC.

### EMBL Heidelberg
**Visiting Scholar** · 2020 · Heidelberg, Germany

Anna Kreshuk's group. Automated localization of kinetochores in cell division microscopy data.

### CNRS CINAM
**Research Engineer (CDD)** · 2019 — 2020 · Marseille, France

Developed automated pipeline for detection and classification of red blood cells from blood flow recordings. Prototype for ICOVELL spin-off.

### European Centre for Medium-Range Weather Forecasts (ECMWF)
**ESoWC Fellow** · 2020 & 2021 · Remote

Deep learning methods for tropical cyclone detection and tracking in reanalysis data.

### CERN-HSF
**Google Summer of Code Student** · 2018 · Remote

Built a package manager for the ROOT data analysis framework.

---

## Education

**M.Sc. Computer Science** — Simon Fraser University, 2023

**B.E. Computer Engineering** — Pune Institute of Computer Technology, University of Pune, 2019

---

## Selected Publications

See [Publications](/publications/) for the full list.

---

## Skills

**ML/DL**: PyTorch, MONAI, PyG, JAX, scikit-learn, self-supervised learning, graph neural networks, equivariant networks, diffusion models

**Computer Vision**: 3D microscopy, segmentation, instance detection, super-resolution, medical image analysis

**Infrastructure**: Neo4j, Docker, distributed training (multi-GPU), MCP tools, CI/CD

**Languages**: Python, TypeScript, C++, Rust (learning)

**Domains**: Cell biology, astrophysics, climate science, medical imaging

---

*Download PDF version: [CV.pdf](/files/cv.pdf)* (coming soon)
